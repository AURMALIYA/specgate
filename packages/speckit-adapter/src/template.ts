...
/** Generate the `/speckit.gate` command prompt the agent runs before generation. */
export function generateGateCommand(config: SpecGateConfig): string {
  return [
    "---",
    ...
    `2. Run: \`specgate-speckit gate specs/<feature> --config <org-config>\` (org: ${config.org}).`,
